...
export class ManagerRegisterDoctorComponent implements OnInit {
  ...
  constructor(private serviceSpecialization: SpecializationService, private serviceCheck: CheckService, private serviceDoctor: DoctorService) {
  }
  ...
  speclizationlist() {
    ...
    this.doctor._id = Math.random().toString(36)

    this.serviceDoctor.create(this.doctor).subscribe((resp: Object) => {
        if (resp['message'] == 'ok') {
      ...
      (error: HttpErrorResponse) => {
        this.message = "Email id or Username already used."
        this.alert.style.visibility = "visible"
      }
    );
  }
  ...
  onSelectFile(event) {
    if (event.target.files) {
      ...
      reader.onload = (event: any) => {
        ...
        let img = new Image()
        img.src = this.doctor.image
        img.onload = () => {
          if (img.height > 300 || img.width > 300) {
            this.message = "Size should be 300x300px."
            this.alert.style.visibility = "visible"
            this.doctor.image = this.defaultImage
          } else if (img.height < 100 || img.width < 100) {
            this.message = "Size should be minimum 100*100"
            this.alert.style.visibility = "visible"
            this.doctor.image = this.defaultImage
          }
        }
